...
def test_gateway_rejects_anonymous_request_on_protected_route(monkeypatch):
    """Anonymous requests must be rejected when auth is required."""
...
def test_gateway_admin_route_blocks_normal_user(monkeypatch):
    """Admin-only routes must reject non-admin users."""
    monkeypatch.setenv("PROJECTRAG_LOCAL_ROLE", "viewer")
    monkeypatch.setattr(settings, "auth_required", False)
    monkeypatch.setattr(settings, "enforce_rbac", True)
    monkeypatch.setattr(settings, "rate_limit_per_minute", 0)
    ...
    assert response.status_code == 403
    assert response.json()["detail"] == "Forbidden"
    assert response.json()["policy_decision"]["permission"] == "admin"
    assert response.json()["policy_decision"]["role"] == "viewer"


def test_gateway_connector_route_requires_ingest_permission(monkeypatch):
    """Connector sync routes must require ingest permission."""
    monkeypatch.setenv("PROJECTRAG_LOCAL_ROLE", "viewer")
    monkeypatch.setattr(settings, "auth_required", False)
    monkeypatch.setattr(settings, "enforce_rbac", True)
    monkeypatch.setattr(settings, "rate_limit_per_minute", 0)
    ...
    @app.post("/connectors/aws/sync")
    async def sync_connector(request: Request):
        return {"ok": True}

    client = TestClient(app)
    response = client.post("/connectors/aws/sync", json={"dry_run": True})
...
def test_gateway_connector_route_allows_admin(monkeypatch):
    """Admin users can access connector sync routes."""
