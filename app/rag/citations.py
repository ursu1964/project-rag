...
def _vector_citations(rows: list[Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        citations.append(
            {
                "id": str(row.get("citation_id") or f"V{index}"),
                "type": "vector",
                "source": str(source),
...
def _memory_citations(rows: list[Any]) -> list[dict[str, Any]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        citations.append(
            {
                "id": str(row.get("citation_id") or f"M{index}"),
                "type": "memory",
...
def _graph_citations(graph: Any) -> list[dict[str, Any]]:
    if isinstance(graph, list):
        citations: list[dict[str, Any]] = []
        for index, row in enumerate(graph, start=1):
            if not isinstance(row, dict):
                continue
            subject = _graph_value(row, "subject") or _graph_value(row, "source")
            predicate = _graph_value(row, "predicate") or _graph_value(row, "relationship") or _graph_value(row, "section")
            obj = _graph_value(row, "object") or _graph_value(row, "target")
            fact = " ".join(part for part in (subject, predicate, obj) if part)
            citations.append(
                {
                    "id": str(row.get("citation_id") or f"G{index}"),
                    "type": "graph",
                    "source": str(subject or "graph"),
