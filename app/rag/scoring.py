...
def score_vector_result(distance) -> float:  # noqa: ANN001
    try:
...
def score_graph_result(result) -> float:  # noqa: ANN001
    if not result:
        return 0.0
    if isinstance(result, dict):
        if result.get("subject") or result.get("object") or result.get("fact_text"):
            return 1.0
...
def score_memory_result(result) -> float:  # noqa: ANN001
    if not result:
        return 0.0
    if isinstance(result, dict):
        return 1.0 if result.get("value") or result.get("key") or result.get("content") else 0.5
    return 1.0


def weighted_score(route: str, vector_score: float, graph_score: float, memory_score: float, metadata_score: float = 0.0) -> float:
