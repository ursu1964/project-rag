...
_DEFAULT_MIN_EVIDENCE_SCORE = 0.05
...
def _attach_citation_ids(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    counters = {"vector": 0, "graph": 0, "memory": 0}
    prefixes = {"vector": "V", "graph": "G", "memory": "M"}
    annotated: list[dict[str, Any]] = []
    for item in items:
        evidence_type = str(item.get("evidence_type") or "")
        prefix = prefixes.get(evidence_type)
        if not prefix:
            annotated.append(item)
            continue
        counters[evidence_type] += 1
        annotated.append({**item, "citation_id": f"{prefix}{counters[evidence_type]}"})
    return annotated
...
def run(state: dict) -> dict:
    ...
    min_score = float(state.get("min_evidence_score", _DEFAULT_MIN_EVIDENCE_SCORE))
    ...
    scored_vector = [item for item in scored_vector if float(item.get("evidence_score") or 0.0) >= min_score]
    scored_graph = [item for item in scored_graph if float(item.get("evidence_score") or 0.0) >= min_score]
    scored_memory = [item for item in scored_memory if float(item.get("evidence_score") or 0.0) >= min_score]

    scored_vector = _attach_citation_ids(scored_vector)
    scored_graph = _attach_citation_ids(scored_graph)
    scored_memory = _attach_citation_ids(scored_memory)
